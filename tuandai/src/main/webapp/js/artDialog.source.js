...
artDialog.fn = artDialog.prototype = {
	...
	_init: function (config) {
        ...
        var currentHeight=config.height;
		if(that.config.minWidth>that.DOM.wrap[0].offsetWidth)
            {
            currentWidth=that.config.minWidth;
            }
            if(that.config.minHeight+DOM.main[0].offsetHeight>that.DOM.wrap[0].offsetHeight)
            {
            currentHeight=that.config.minHeight;
            }
        ...
        that.size(currentWidth,currentHeight);
		config.follow
	...
	lock: function () {
		...
		if (_isIE6) lockMask.html(
			'<iframe src="empty.html" style="width:100%;height:100%;position:absolute;' +
...
//        else
//        {
//        wrap.innerHTML = artDialog.simpletemplates;
//        }
 ...
 * 自定义无任何边框弹出框
 */
artDialog.simpletemplates = 
'<div class="aui_outer" style="border:0px;">' +
	'<table class="aui_border">' +
		'<tbody>' +
			...
			'<tr>' +
				...
				'<td class="aui_c">' +
					'<div class="aui_inner" style="background:transparent none repeat scroll 0% 0%;">' +
					'<table class="aui_dialog">' +
						'<tbody>' +
					...
					'</div>' +
				'</td>' +
				'<td class="aui_e" style="filter:alpha(opacity=0); width:0px; height:0px;"></td>' +
			'</tr>' +
			'<tr>' +
				'<td class="aui_sw" style="filter:alpha(opacity=0);width:0px; height:0px;"></td>' +
				'<td class="aui_s" style="filter:alpha(opacity=0); width:0px; height:0px;"></td>' +
				'<td class="aui_se" style="filter:alpha(opacity=0); width:0px; height:0px;"></td>' +
			'</tr>' +
		'</tbody>' +
	'</table>' +
'</div>';
...
artDialog.defaults = {
	...
	left: '50%',				// X轴坐标
	top: '38.2%',				// Y轴坐标
	zIndex: 1000000,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
...
//全局提示层
function globalDialog(newTitle, strmsg, strurl, delall) {
    if (delall == "1") {
        //删除其他弹出层
    ...
    if (strurl == "") {
        art.dialog({
            ...
            width: 300,
            zIndex: 9999999,
            content: "<div style='font-size:13px;padding: 5px 10px;line-height: 21px;color:gray;'>" + strmsg + "</div>",
            ok: function () { }
        });
    }
    else {
