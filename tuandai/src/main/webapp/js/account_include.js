    ...
    for (var i = 0; i < dlist.length; i++) {
        if (dlist[i].cycDate.toString() == date.toString()) {
            ...
            if (parseFloat(dlist[i].inAmount) > 0) {
                d += "<br/><a href='" + __p2pWebsiteUrl + "/member/account_loanReturnPlanDetail?date=" + sDate + "' target='_blank'>回款：" + fmoney(parseFloat(dlist[i].inAmount).toFixed(2), 2) + "元</a>";
                obj['in'] = 1;
            }
            if (parseFloat(dlist[i].outAmount) > 0) {
                d += "<br/><a href='" + __p2pWebsiteUrl + "/member/account_borrowReturnPlanDetail?date=" + sDate + "' target='_blank'>结清：" + fmoney(parseFloat(dlist[i].outAmount).toFixed(2), 2) + "元</a>";
                obj['out'] = 1;
            }
            ...
            if (parseFloat(dlist[i].dqInAmount) > 0) {
                d += "<br/><a href='" + __DQWebsiteUrl + "/member/account_loanReturnPlanDetail?date=" + sDate + "' target='_blank'>回款：" + fmoney(parseFloat(dlist[i].dqInAmount).toFixed(2), 2) + "元</a>";
                obj['in'] = 1;
            }
            if (parseFloat(dlist[i].dqOutAmount) > 0) {
                d += "<br/><a href='" + __DQWebsiteUrl + "/member/account_borrowReturnPlanDetail?date=" + sDate + "' target='_blank'>还款：" + fmoney(parseFloat(dlist[i].dqOutAmount).toFixed(2), 2) + "元</a>";
                obj['out'] = 1;
            }
            if (parseFloat(dlist[i].zxInAmount) > 0 || parseFloat(dlist[i].zxOutAmount) > 0) {
                if (d.indexOf("网贷") != -1 || d.indexOf("定期") != -1)
                    d += '<br/>';
                d += "<span>智享</span>";
            }
            if (parseFloat(dlist[i].zxInAmount) > 0) {
                d += "<br/><a href='" + __ZXWebsiteUrl + "/member/zshare/account_loanReturnPlanDetail?date=" + sDate + "' target='_blank'>回款：" + fmoney(parseFloat(dlist[i].zxInAmount).toFixed(2), 2) + "元</a>";
                obj['in'] = 1;
            }
            if (parseFloat(dlist[i].zxOutAmount) > 0) {
                d += "<br/><a href='" + __ZXWebsiteUrl + "/member/account_zxborrowReturnPlanList?date=" + sDate + "' target='_blank'>结清：" + fmoney(parseFloat(dlist[i].zxOutAmount).toFixed(2), 2) + "元</a>";
                obj['out'] = 1;
            }
            d += "</div>";
            break;
